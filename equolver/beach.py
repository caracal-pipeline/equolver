...
#            print(repr(self._cubes[i][0].header))
    ...
    def gentrans(self, tra_modelnames=None, tra_residualnames=None,
        ...
        plane number (Fortran/FITS style). If a plane is partially
        nan, the flagged pixels are treated as 0 and are flagged again
        in the output cube.
        ...
        for i in range(len(transn)):
            ...
            incubus[0].header['DATAMAX'] = np.nanmax(incubus[0].data)
            incubus[0].header['DATAMIN'] = np.nanmin(incubus[0].data)
